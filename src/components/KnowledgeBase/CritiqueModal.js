...
const CritiqueModal = ({ critique, onClose, onUpdate }) => {
  ...
  console.log("Critique in modal:", critique);

  const handleSave = async () => {
  try {
    // Prepare the data to be sent
    const dataToSend = {
      ...critique,  // Keep the original structure
      content: editedCritique.content,  // Only update the content field
  ...
  setEditedCritique(prevState => ({
    ...
    content: {
      ...prevState,
      [field]: [...(prevState[field] || []), field === 'category_critiques' ? { category: '', critique: '' } : '']
    }
  }));
};
...
const removeItem = (field, index) => {
  ...
  const EditableField = ({ value, onChange, multiline = false }) => {
    ...
    if (!isEditing) {
      return (
        ...
        >
          <p className="text-gray-700 p-2 bg-purple-50 rounded-md">
            {value}
          </p>
  ...
  console.log("critique to be rendered: ", editedCritique);
